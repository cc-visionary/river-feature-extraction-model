...
def median_filter(img: np.ndarray,
    ...
    for channel in range(3):
        pixel_values[channel] = list(
            img[index[0], index[1], channel] for index in indices)
...
def process_regions(org_image: np.ndarray,
    ...
    non_shadow_kernel = cv.getStructuringElement(
        cv.MORPH_ELLIPSE, non_shadow_kernel_size)
        ...
        if not label == 0:
            ...
            if cv.countNonZero(temp_filter) >= shadow_size_threshold:
                ...
                shadow_clear_img = edge_median_filter(cv.cvtColor(shadow_clear_img, cv.COLOR_BGR2HSV),
                                                      contours)
                if verbose:
                    display_region(org_image, shadow_clear_img,
                                   label, temp_filter, contours)
